...
public class ShoppingCart {
    ...
    public void add(String input, String dirPath, String stringLogin) throws IOException {
        ...
        String stringVal;
        try (Scanner scan = new Scanner(input.substring(4)); FileWriter fileWriter = new FileWriter(dirPath + File.separator + stringLogin, true); PrintWriter printWriter = new PrintWriter(fileWriter)) {

            while (scan.hasNext()) {
                stringVal = scan.next();
                if (!cartItems.isEmpty() && cartItems.contains(stringVal)) {
    ...
    public void delete(String input, String dirPath, String stringLogin) throws IOException{
        String stringVal;
        try (Scanner scan = new Scanner(input.substring(6))) {
            while (scan.hasNext()) {
                stringVal = scan.next();
        ...
        try (FileWriter fileWriter = new FileWriter(dirPath + File.separator + stringLogin, false); BufferedWriter bwr = new BufferedWriter(fileWriter)) {
