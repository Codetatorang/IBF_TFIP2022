...
public class FixedDepositAccount extends BankAccount {
    ...
    public Boolean getIsSetInterest() {
        return isSetInterest;
    }

    private void setIsSetInterest(Boolean isSetInterest) {
        this.isSetInterest = isSetInterest;
    }

    public Boolean getIsSetDuration() {
        return isSetDuration;
    }

    private void setIsSetDuration(Boolean isSetDuration) {
        this.isSetDuration = isSetDuration;
    }
    ...
    void setInterest(float interest) throws IllegalArgumentException {
        if (!isSetInterest) {
            this.interest = interest;
            setIsSetInterest(true);
            System.out.printf("sucessfully set interest rate to %.1f%%\n", this.interest);
        } else {
            throw new IllegalArgumentException("Cannot adjust interest again. \n");
        }
    }

    public int getDurInMonths() {
        return durInMonths;
    }

    private void setDurInMonths(int durInMonths) {
        if (!isSetDuration) {
            this.durInMonths = durInMonths;
            setIsSetDuration(true);
            System.out.printf("sucessfully set duration to %d months \n", this.durInMonths);
        } else {
            throw new IllegalArgumentException("Cannot adjust duration again. \n");
        }
